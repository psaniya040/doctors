...
router.get('/availability', async (req, res) => {
    ...
    try {
        ...
        res.status(200).json({ success: true, date, doctorId, openSlots });
    } catch (error) {
...
router.post('/book', async (req, res) => {
    ...
    try {
        ...
        res.status(201).json({
            ...
            appointmentDetails: { date, timeSlot, doctorId, patientId, link: consultationLink }
        });
    } catch (error) {
        console.error('Booking Error:', error);
        res.status(500).json({ success: false, message: 'Failed to schedule appointment. Slot may be taken.' });
    }
});
